...
This module is also importable by backend/server.py for headless (in-app) capture:
    from capture_bottles import ThreadedCapture, prepare_dataset_dirs, write_data_yaml,
                                 save_capture, next_run_dir, BOX_PRESETS, TOTAL_CAPTURES
    # run_capture() now accepts headless=False, box_type="tall"

Usage:
    python capture_bottles.py --weights runs/detect/runs/train_v10/weights/best.pt

    #hotspot
    python capture_bottles.py --weights runs/detect/runs/train_v10/weights/best.pt --source http://192.168.0.211:81/stream

    #home
    python capture_bottles.py --weights runs/detect/runs/train_v10/weights/best.pt --source http://192.168.0.31:81/stream


    python capture_bottles.py --weights runs/detect/runs/train_v10/weights/best.pt --epochs 30 --device gpu

    # Train-only (skip capture, run training on existing dataset):
    python capture_bottles.py --class-name john_tylenol --weights ... --train-only
    ...
    def _update(self):
        while not self.stopped:
            ...
            if ret and frame is not None:
                # .copy() is required: CAP_FFMPEG reuses its internal decode buffer,
                # so storing the raw reference means self.frame is overwritten on the
                # next cap.read() call — producing dark/blank frames in the server.
                with self._lock:
                    self.ret   = True
...
def run_capture(
    ...
    can_capture              = False # initialised here so key-handler is safe in all paths
    ...
    while captures_done < TOTAL_CAPTURES:
        ...
        if headless and frame is not None and captures_done < TOTAL_CAPTURES:
            p      = BOX_PRESETS[selected_key]
            rfh, rfw = frame.shape[:2]
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="tall",
        help="Bottle shape preset; sets the fixed selection in --headless mode and the initial selection in interactive mode (default: tall)",
    )
    ...
    if args.train_only:
        print("=" * 60)
        print("  Pill Bottle Training (train-only mode)")
        print("=" * 60)
        if not dataset_dir.exists():
            print(f"[ERROR] Dataset directory not found: {dataset_dir}")
            return 2
        total_saved = sum(1 for _ in (dataset_dir / "images").rglob("*.jpg")) if (dataset_dir / "images").exists() else 0
        if total_saved == 0:
        ...
        print("=" * 60)
        print(f"  Done! Fine-tuned model: {best_pt}")
        print("=" * 60)
        return 0

    # ── Full capture + train mode ─────────────────────────────────────────────
    print("=" * 60)
